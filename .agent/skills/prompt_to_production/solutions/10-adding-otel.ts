    ...
    async run(currentInput: string | Interactions.Step[] | Interactions.Content[]): Promise<Interactions.Interaction> {
        ...
        const response: Interactions.Interaction = await logfire.span(
            'Gemini API Call',
            {
                ...
                input: JSON.stringify(currentInput, null, 2)
            },
            {},
            async () => {
        ...
        fs.appendFileSync("10-adding-otel.log", JSON.stringify(response, null, 2) + "\n\n", "utf-8");
            ...
            async () => {
                for (const step of response.steps || []) {
                    if (step.type === "function_call") {
                        ...
                        logfire.info('tool-response', { result: resultStr });
